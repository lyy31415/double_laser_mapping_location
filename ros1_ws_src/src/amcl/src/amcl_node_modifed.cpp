...
#define LOOKUPTRANSFORM        0
#define MANUAL_TRANSFORM       0
#define MANUAL_SMOOTHER        0
#define SMOOTH_X_THRESHOLD     0.01 // 单位：米
#define SMOOTH_Y_THRESHOLD     0.01 // 单位：米
#define SMOOTH_DIST_THRESHOLD  0.01414
#define SMOOTH_ANGLE_THRESHOLD (0.5 / RAD_TO_DEGREE) // 单位：弧度
...
class AmclNode
{
  public:
    ...
    void fast_pub_amclpose_fun();
    void run();
    void pose_smoother(const geometry_msgs::PoseWithCovarianceStamped &fast_map_to_baselink, 
                   geometry_msgs::PoseWithCovarianceStamped &fast_map_to_baselink_smoother);
  private:
  ...
  //beam skip related params
    ...
    geometry_msgs::PoseWithCovarianceStamped fast_pub_amclpose_;// added by lyy
    bool pose_smoother_init_;
    std::queue<geometry_msgs::PoseWithCovarianceStamped> transformstamped_que_;
    bool has_resampled_;
    pf_vector_t delta_odom_pose_;
    ros::Subscriber odom_sub_;
    nav_msgs::Odometry* odom_data_;
    nav_msgs::Odometry old_odom_data_;
    geometry_msgs::PoseWithCovarianceStamped map_pose_;
    tf2::Transform old_map_to_odom_transform_;
    geometry_msgs::PoseStamped odom_to_map;
    bool latest_odom_to_map_valid_;
    bool new_to_old_transform_init_;
};
...
main(int argc, char** argv)
{
  ...
  boost::thread fast_pub_Thread(&AmclNode::run, amcl_node_ptr);
  if (argc == 1)
  {
    // run using ROS input
    ros::spin();
    ...
    // {
    //   ros::spinOnce();
    //   amcl_node_ptr->fast_pub_amclpose_fun();
    //   loop_rate.sleep();
    // }
    // tf::StampedTransform transform;
    // amcl_node_ptr->listener.lookupTransform("/turtle2", "/turtle1", ros::Time(0), transform);
  }
...
AmclNode::AmclNode() :
        ...
        pose_smoother_init_(false),
        has_resampled_(false),
        latest_odom_to_map_valid_(false),
        new_to_old_transform_init_(false)
{
  ...
  odom_data_ = new nav_msgs::Odometry;
  ...
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 1000, true);
  ...
  odom_sub_ = nh_.subscribe("odom", 2, &AmclNode::odomReceived, this);
...
void AmclNode::runFromBag(const std::string &in_bag_fn)
{
  ...
  ros::Publisher laser_pub = nh_.advertise<sensor_msgs::LaserScan>(scan_topic_name, 1000);
  ros::Publisher tf_pub = nh_.advertise<tf2_msgs::TFMessage>("/tf", 1000);
...
void AmclNode::savePoseToServer()
{
  ...
  tf2::convert(latest_odom_pose_.pose, odom_pose_tf2);// latest_odom_pose_ 表示 odom_to_baselink 位姿
...
AmclNode::laserReceived_1(const sensor_msgs::LaserScanConstPtr& laser_scan_1)
{
  ...
  // Do we have the base->base_laser Tx yet? 第一帧激光数据进来，确定激光雷达相对base_link的位置和姿态
  ...
  if(lasers_update_1[laser_index_1])
  {
    ...
    double angle_min = tf2::getYaw(min_q.quaternion);// 在 base_frame_id_ 下的角度
    double angle_increment = tf2::getYaw(inc_q.quaternion) - angle_min;
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      try
      {
        ...
        latest_odom_to_map_valid_ = true;
      }
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  {
    ros::Time now = ros::Time::now();
    // wait a little for the latest tf to become available，查看从msg时刻到现在的，在odom_frame_id_坐标系下的增量
...
void AmclNode::fast_pub_amclpose_fun()
{
  ros::Time now = ros::Time::now();

  #if LOOKUPTRANSFORM
    geometry_msgs::TransformStamped fast_map_to_baselink;

    try
    {
      fast_map_to_baselink = tf_->lookupTransform(global_frame_id_,base_frame_id_,ros::Time(0),ros::Duration(0.5));
      // fast_odom_to_baselink = tf_->lookupTransform(odom_frame_id_,base_frame_id_,ros::Time(0));
    }
    catch(tf2::TransformException)
    {
      ROS_ERROR("Failed to get lookupTransform!");
      // return;
    }

    double tmp_yaw;
    tmp_yaw = tf2::getYaw(fast_map_to_baselink.transform.rotation) * RAD_TO_DEGREE;
    ROS_INFO("fast_map_to_baselink   %.3f m, %.3f m, %.3f deg, with stamp=%f, frame_id=%s, child_frame_id=%s",
            fast_map_to_baselink.transform.translation.x,
            fast_map_to_baselink.transform.translation.y,
            tmp_yaw, fast_map_to_baselink.header.stamp.toSec(),
            fast_map_to_baselink.header.frame_id.c_str(),
            fast_map_to_baselink.child_frame_id.c_str()
            );

    // geometry_msgs::PoseWithCovarianceStamped fast_pub_amclpose_
    // fast_pub_amclpose_.header.stamp = now;
    // fast_pub_amclpose_.header.frame_id = fast_map_to_baselink.header.frame_id;

    fast_pub_amclpose_.header = fast_map_to_baselink.header;
    fast_pub_amclpose_.pose.pose.position.x = fast_map_to_baselink.transform.translation.x;
    fast_pub_amclpose_.pose.pose.position.y = fast_map_to_baselink.transform.translation.y;
    fast_pub_amclpose_.pose.pose.orientation = fast_map_to_baselink.transform.rotation;

  #else
    // geometry_msgs::PoseWithCovarianceStamped ident;
    geometry_msgs::PoseWithCovarianceStamped pose_smoother_tmp;
    pf_vector_t delta_odom_data;
    ...
    try
    {
      ...
      this->tf_->transform(ident, map_pose_, global_frame_id_,ros::Duration(0.5));
      if(has_resampled_ == true)
      {
        has_resampled_ = false;
        map_pose_ = fast_pub_amclpose_;
        std::cout<<" ################### Resampled pose is received! ###################"<<std::endl;
      }
      else
      {
        tf2::Transform odom_to_basefootprint_transform, modified_odom_to_basefootprint_transform;
        tf2::Transform odom_to_map_transform, new_to_old_transform;

        if(latest_odom_to_map_valid_ == true)
        {
          tf2::convert(odom_to_map.pose, odom_to_map_transform);
        }

        if(new_to_old_transform_init_ = false)
        {
          new_to_old_transform_init_ = true;
          new_to_old_transform.setIdentity();
        }
        else
        {
          new_to_old_transform = odom_to_map_transform * old_map_to_odom_transform_;
        }
          
        tf2::Vector3 origin(odom_data_->pose.pose.position.x, odom_data_->pose.pose.position.y, 0.0);
        tf2::Quaternion q;
        odom_to_basefootprint_transform.setOrigin(origin);
        tf2::convert(odom_data_->pose.pose.orientation, q);
        odom_to_basefootprint_transform.setRotation(q);

        modified_odom_to_basefootprint_transform = new_to_old_transform * odom_to_basefootprint_transform;
        odom_data_->pose.pose.position.x = modified_odom_to_basefootprint_transform.getOrigin().x();
        odom_data_->pose.pose.position.y = modified_odom_to_basefootprint_transform.getOrigin().y();
        odom_data_->pose.pose.position.z = 0.0;
        tf2::convert(modified_odom_to_basefootprint_transform.getRotation(), odom_data_->pose.pose.orientation);

        old_map_to_odom_transform_ = odom_to_map_transform.inverse();


        // std::cout<<"odom_data_->pose.pose.position.x: "<< odom_data_->pose.pose.position.x << std::endl;
        delta_odom_data.v[0] = odom_data_->pose.pose.position.x - old_odom_data_.pose.pose.position.x;
        delta_odom_data.v[1] = odom_data_->pose.pose.position.y - old_odom_data_.pose.pose.position.y;
        delta_odom_data.v[2] = angle_diff(tf2::getYaw(odom_data_->pose.pose.orientation), tf2::getYaw(old_odom_data_.pose.pose.orientation));

        if(sqrt(delta_odom_data.v[1]*delta_odom_data.v[1] + 
                delta_odom_data.v[0]*delta_odom_data.v[0]) < 0.01)
          delta_rot1 = 0.0;
        else
          delta_rot1 = angle_diff(atan2(delta_odom_data.v[1], delta_odom_data.v[0]),
                                  tf2::getYaw(old_odom_data_.pose.pose.orientation));
        delta_trans = sqrt(delta_odom_data.v[0]*delta_odom_data.v[0] +
                          delta_odom_data.v[1]*delta_odom_data.v[1]);
        delta_rot2 = angle_diff(delta_odom_data.v[2], delta_rot1);

        delta_odom_pose_.v[0] = delta_trans * cos(delta_odom_pose_.v[2] + delta_rot1);
        delta_odom_pose_.v[1] = delta_trans * sin(delta_odom_pose_.v[2] + delta_rot1);
        delta_odom_pose_.v[2] = delta_rot1 + delta_rot2;

        tf2::Quaternion tmp_q;
        map_pose_.header.stamp = ros::Time::now();
        map_pose_.pose.pose.position.x += delta_odom_pose_.v[0];
        map_pose_.pose.pose.position.y += delta_odom_pose_.v[1];
        double yaw = normalize(tf2::getYaw(map_pose_.pose.pose.orientation) + delta_odom_pose_.v[2]);
        tmp_q.setRPY(0.0, 0.0, yaw);
        tf2::convert(tmp_q, map_pose_.pose.pose.orientation);
        
        old_odom_data_ = *odom_data_;

        // std::cout<<"map_pose_.pose.pose.position.x: "<<map_pose_.pose.pose.position.x<<std::endl;
      }
    }
    catch(tf2::TransformException)
    {
      ROS_ERROR("Failed to get transform!");
      // return;
    }
    
    // tf2::convert(map_pose_.header, fast_pub_amclpose_.header);
    // tf2::convert(map_pose_.pose.pose, fast_pub_amclpose_.pose.pose);

  #endif

  #if MANUAL_TRANSFORM
    geometry_msgs::TransformStamped fast_map_to_baselink_m;
    geometry_msgs::TransformStamped fast_odom_to_baselink;
    tf2::Transform map_to_odom;
    tf2::Transform odom_to_baselink;
    tf2::convert(latest_tf_.inverse(), map_to_odom);
    tf2::convert(fast_odom_to_baselink.transform, odom_to_baselink);
    tf2::convert(map_to_odom * odom_to_baselink, fast_map_to_baselink_m.transform);

    fast_map_to_baselink_m.header.frame_id = global_frame_id_;
    fast_map_to_baselink_m.header.stamp = fast_odom_to_baselink.header.stamp;
    fast_map_to_baselink_m.header.seq = fast_odom_to_baselink.header.seq;
    fast_map_to_baselink_m.child_frame_id = base_frame_id_;

    double tmp_yaw_m;
    tmp_yaw_m = tf2::getYaw(fast_map_to_baselink_m.transform.rotation) * RAD_TO_DEGREE;
    ROS_INFO("fast_map_to_baselink_m %.3f m, %.3f m, %.3f deg, with stamp=%f, frame_id=%s, child_frame_id=%s\n",
            fast_map_to_baselink_m.transform.translation.x,
            fast_map_to_baselink_m.transform.translation.y,
            tmp_yaw_m, fast_map_to_baselink_m.header.stamp.toSec(),
            fast_map_to_baselink_m.header.frame_id.c_str(),
            fast_map_to_baselink_m.child_frame_id.c_str()
            );

    fast_pub_amclpose_.header = fast_map_to_baselink_m.header;
    fast_pub_amclpose_.pose.pose.position.x = fast_map_to_baselink_m.transform.translation.x;
    fast_pub_amclpose_.pose.pose.position.y = fast_map_to_baselink_m.transform.translation.y;
    fast_pub_amclpose_.pose.pose.orientation = fast_map_to_baselink_m.transform.rotation;

  #endif

  // pose_smoother(fast_pub_amclpose_, pose_smoother_tmp);
  pose_smoother(map_pose_, pose_smoother_tmp);

  #if !LOOKUPTRANSFORM
    double tmp_yaw_t;
    tmp_yaw_t = tf2::getYaw(pose_smoother_tmp.pose.pose.orientation) * RAD_TO_DEGREE;
    ROS_INFO("smoother map_to_baselink: %.3f m, %.3f m, %.3f deg, with stamp=%f, frame_id=%s",
          pose_smoother_tmp.pose.pose.position.x,
          pose_smoother_tmp.pose.pose.position.y,
          tmp_yaw_t, pose_smoother_tmp.header.stamp.toSec(),
          pose_smoother_tmp.header.frame_id.c_str()
          );
  #endif



  pose_pub_.publish(pose_smoother_tmp);
}

void AmclNode::pose_smoother(const geometry_msgs::PoseWithCovarianceStamped &fast_map_to_baselink, 
                   geometry_msgs::PoseWithCovarianceStamped &fast_map_to_baselink_smoother)
{
  #if MANUAL_SMOOTHER
  // 构建长度为 n 的滑动窗口，对输入数据进行平滑处理
  geometry_msgs::PoseWithCovarianceStamped transformstamped_tmp;
  tf2::Vector3 pos_que_back, pos_step, tmp_pos_current;
  tf2::Quaternion quat_que_back, quat_step, tmp_quat_current;

  // 把第一个数据先入队
  if(pose_smoother_init_ == false)
  {
    pose_smoother_init_ = true;
    transformstamped_que_.push(fast_map_to_baselink);
  }
  else
  {
    tf2::convert(fast_map_to_baselink.pose.pose.position, tmp_pos_current);
    tf2::convert(fast_map_to_baselink.pose.pose.orientation, tmp_quat_current);
    tf2::convert(transformstamped_que_.back().pose.pose.position, pos_que_back);
    tf2::convert(transformstamped_que_.back().pose.pose.orientation, quat_que_back);

    double dist_tmp = pos_que_back.distance(tmp_pos_current);
    double angle_tmp = fabs(angle_diff(tf2::getYaw(tmp_quat_current), tf2::getYaw(quat_que_back)));

    if(dist_tmp > SMOOTH_DIST_THRESHOLD || angle_tmp > SMOOTH_ANGLE_THRESHOLD)
    {
      int dist_step_num = std::ceil(dist_tmp / SMOOTH_DIST_THRESHOLD);
      int angle_step_num = std::ceil(angle_tmp / SMOOTH_ANGLE_THRESHOLD);
      int max_step_num  = std::max(dist_step_num, angle_step_num);

      std::cout<<" max_step_num: "<<max_step_num<<std::endl;
      std::cout<<" fast_map_to_baselink: ";
      std::cout<<fast_map_to_baselink.pose.pose.position.x;
      std::cout<<", "<<fast_map_to_baselink.pose.pose.position.y;
      std::cout<<std::endl<<std::endl;

      if(max_step_num > 1)
      {
        double step = 1.0 / (max_step_num - 1);

        for(int step_index = 1; step_index < max_step_num; step_index++)
        {
          pos_step = pos_que_back.lerp(tmp_pos_current, step*step_index);
          quat_step = quat_que_back.slerp(tmp_quat_current, step*step_index);

          transformstamped_tmp = fast_map_to_baselink;
          transformstamped_tmp.pose.pose.position.x = pos_step.getX();
          transformstamped_tmp.pose.pose.position.y = pos_step.getY();
          tf2::convert(quat_step, transformstamped_tmp.pose.pose.orientation);
          transformstamped_tmp.header.stamp = ros::Time::now();
          transformstamped_que_.push(transformstamped_tmp);
        }
      }
      else
      {
        ROS_ERROR("max_step_num is not greater than 1!");
      }
    ...
    fast_map_to_baselink_smoother = transformstamped_que_.front();
    std::cout<<" ===========transformstamped_que_ length is: "<<transformstamped_que_.size();
    std::cout<<" ==========="<<std::endl;

    if(transformstamped_que_.size() > 5)
    {
      for(int i = 0; i < 5; i++)
      {
        if(!transformstamped_que_.empty())
          transformstamped_que_.pop();
        else
          ROS_ERROR("transformstamped_que_  is empty, cannot pop 5 times!"); 
      }
    }
    else if(transformstamped_que_.size() > 2)
    {
      for(int i = 0; i < 2; i++)
      {
        if(!transformstamped_que_.empty())
          ...
          ROS_ERROR("transformstamped_que_  is empty, cannot pop 2 times!"); 
      }
    }
    else
    {
      if(!transformstamped_que_.empty())
        ...
        ROS_ERROR("transformstamped_que_  is empty, cannot pop!"); 
    }
  }
  #else
    tf2::TransformStorage transformstorage;
    transformstorage.translation_.setValue();
  #endif
}


void AmclNode::odomReceived(const nav_msgs::OdometryConstPtr& msg)
{
  // odom callback
  tf2::convert(*msg, *odom_data_);
  // odom_data_ = const_cast<nav_msgs::OdometryConstPtr>(msg);
  // std::cout<<"odom receieved!"<<std::endl;
}
