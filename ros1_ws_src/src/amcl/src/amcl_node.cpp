...
#define SMOOTH_DIST_THRESHOLD  0.008
...
normalize_quat(tf2::Quaternion q_in)
{
  tf2::Quaternion q_norm;
  double angle = normalize(tf2::getYaw(q_in));
  q_norm.setRPY(0.0, 0.0, angle);
...
class AmclNode
{
  ...
  //beam skip related params
    ...
    bool has_sendTransform_;
    pf_vector_t delta_sum_;
        ...
        has_initial_pose_(false)
{
  ...
  pose_test_pub_ = nh_.advertise<amcl::pose_test>("pose_test", 1000, true);
  easy_pose_pub_ = nh_.advertise<amcl::pose_test>("easy_amcl_pose", 1000, true);
...
AmclNode::laserReceived_1(const sensor_msgs::LaserScanConstPtr& laser_scan_1)
{
  ...
  if(pf_init_)
  {
    ...
    // delta_sum_.v[0] += delta.v[0];
    // delta_sum_.v[1] += delta.v[1];
    // delta_sum_.v[2] += delta.v[2];
  ...
  if(resampled || force_publication)
  {
    ...
    std::cout<<" pf_->sets[pf_->current_set].cluster_count: "<<pf_->sets[pf_->current_set].cluster_count<<std::endl;
    for(int hyp_count = 0;
...
void AmclNode::fast_pub_amclpose_fun()
{
  if(has_initial_pose_ == true)
  {
    ...
    amcl::pose_test test_pose;
    amcl::pose_test easy_amcl_pose;
    ...
    try
    {
      ...
      this->tf_->transform(ident, map_pose, global_frame_id_,ros::Duration(0.5));
    }
    catch(tf2::TransformException)
    {
    ...
    // get the cov
    tf2::convert(map_pose.header, fast_pub_amclpose_.header);
    tf2::convert(map_pose.pose.pose, fast_pub_amclpose_.pose.pose);

    pose_smoother(fast_pub_amclpose_, pose_smoother_tmp);

    ros::Time print_now = ros::Time::now();
    if(print_now.toSec() - old_print_time_.toSec() > 1.0)
    {
      old_print_time_ = print_now;
      double tmp_yaw_t;
    ...
    test_pose.header = map_pose.header;
    test_pose.x = map_pose.pose.pose.position.x;
    test_pose.y = map_pose.pose.pose.position.y;
    test_pose.theta = tf2::getYaw(map_pose.pose.pose.orientation) * RAD_TO_DEGREE;
    
    if(has_sendTransform_ == true)
    {
      has_sendTransform_ = false;
      test_pose.laser_update = 1;
    }
    else
    {
      test_pose.laser_update = 0;
    }

    easy_amcl_pose.header = pose_smoother_tmp.header;
    easy_amcl_pose.x = pose_smoother_tmp.pose.pose.position.x;
    easy_amcl_pose.y = pose_smoother_tmp.pose.pose.position.y;
    easy_amcl_pose.theta = tf2::getYaw(pose_smoother_tmp.pose.pose.orientation) * RAD_TO_DEGREE;

    pose_test_pub_.publish(test_pose);
    easy_pose_pub_.publish(easy_amcl_pose);
    pose_pub_.publish(pose_smoother_tmp);
  }
  else
  {
    ROS_WARN("Has no initial pose!\n");
  }
}
                   ...
                   geometry_msgs::PoseWithCovarianceStamped &fast_map_to_baselink_smoother)
{
  ...
  else
  {
    ...
    // tmp_quat_current = normalize_quat(tmp_quat_current);//四元数映射到 -pi ～ pi范围
    // quat_que_back = normalize_quat(quat_que_back);

    double dist_tmp = pos_que_back.distance(tmp_pos_current);
    double tmp_quat_current_yaw = tf2::getYaw(tmp_quat_current);
    double quat_que_back_yaw = tf2::getYaw(quat_que_back);
    double angle_tmp = fabs(angle_diff(tmp_quat_current_yaw, quat_que_back_yaw));
    ...
    if(dist_tmp > SMOOTH_DIST_THRESHOLD || angle_tmp > SMOOTH_ANGLE_THRESHOLD)
    {
      ...
      if(max_step_num > PRINT_THRESHOLD)
      {
    ...
    if(transformstamped_que_.size() > 2)
    {
      for(int i = 0; i < 2; i++)
      {
        if(!transformstamped_que_.empty())
          ...
          ROS_ERROR("transformstamped_que_  is empty, cannot pop 3 times!");
      }
    }
    ...
    if(transformstamped_que_.size() > PRINT_THRESHOLD)
    {
