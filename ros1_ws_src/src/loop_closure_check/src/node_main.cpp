...
//
#include "../include/loop_closure_check/ceres_scan_matcher_2d.h"
#include "../include/loop_closure_check/voxel_filter.h"
#include "../include/loop_closure_check/fast_correlative_scan_matcher_2d.h"
#include <memory>
...
//map中进行接收地图数据，激光直接接收并处理，amcl_pose中做扫描匹配，确定是不是进行branch and bound
...
//***********************fast full map match****************************
    ...
    LoopClosuerCheck::FastCorrelativeScanMatcher(const cartographer::transform::Rigid2d& initial_pose_estimate,const cartographer::sensor::PointCloud&  point_cloud_match,float &score_return)
    {
        ...
        score_return = score;
        std::cout<<std::endl;
        std::cout<<"Branch-and-bound scan matching计算的位置:"<<std::endl;
        std::cout<<transform::ToProto(pose_estimate).DebugString()<<std::endl;
        std::cout<<"Branch-and-bound score is : \n"<<score<<std::endl;
        return match_result;
    }
...
//***********************handle map****************************
    ...
    LoopClosuerCheck::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
    {
        ...
        std::cout<<probability_grid_->limits().GetCellIndex(Eigen::Vector2f(0.f, 0.f))<<std::endl;
        std::cout<<"origin_pose_x : "<<origin_pose_x<<" , origin_pose_y : "<<origin_pose_y<<std::endl;
        std::cout<<probability_grid_->limits().GetCellIndex(Eigen::Vector2f(origin_pose_x, origin_pose_y))<<std::endl;
        ...
        for(int j=0;j<msg.info.height;j++) {
            for (int i = 0; i < msg.info.width; i++) {
                ...
                //        Eigen::Vector2f((i+0.5)*msg.info.resolution + origin_pose_x ,(j+0.5)*msg.info.resolution + origin_pose_y))<<std::endl;
...
//***********************callback***********************
    ...
    LoopClosuerCheck::amclPoseReceived(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
    {
        ...
        auto pose_observation = common::make_unique<transform::Rigid2d>();// 优化后的返回值
        ...
        ceres_scan_matcher_->Match(initial_ceres_pose.translation(), initial_ceres_pose,
                                  ...
                                  &summary);
        std::cout<<"Amcl pose:\n"<<cartographer::transform::ToProto(initial_ceres_pose).DebugString()<<std::endl;
        std::cout<<"Ceres_pose:\n"<<cartographer::transform::ToProto(*pose_observation).DebugString()<<std::endl;
        std::cout<<summary.BriefReport();
        std::cout<<std::endl;

        geometry_msgs::PoseStamped ceres_poseStamped;
        ceres_poseStamped.header = msg->header;
        ceres_poseStamped.pose.position.x = pose_observation->translation()[0];
        ceres_poseStamped.pose.position.y = pose_observation->translation()[1];
        ceres_poseStamped.pose.position.z = 0;
        ceres_poseStamped.pose.orientation = pose_observation->rotation()
        pose_pub_.publish(ceres_poseStamped);


        if(summary.final_cost>100.35f)
        {
            float score=0.f;
            std::cout<<"根据Amcl的定位结果进行优化，cost值大于阈值，开始进行Branch-and-bound scan matching！\n";
            geometry_msgs::PoseStamped poseStamped;

            // Amcl的当前位姿匹配地图得到的误差函数cost值大于阈值，开始进行分支定界全局匹配地图
